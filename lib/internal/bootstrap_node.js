...
(function(process) {
  ...
  // override console with a logger to allow Node.js UWP to redirect output to
  // file and/or debugger output window
  /*global Debug*/
  function overrideConsoleWithNativeLogger() {
    ...
    for (var method in methods) {
      // whenever a console method will be called, we will call the logger with
      ...
      console[method] = (function(methodName, i) {
        ...
        return function() {
          ...
          // Write to logger
          logger.log(logLevel, outputString);
          // Write to stdout incase it was overridden
          process.stdout.write(outputString + '\n');
          // Write to Visual Studio output window if debugger is attached
          if (Debug.debuggerEnabled) {
            Debug.writeln(outputString);
          }
    ...
    console['dir'] = function(object, options) {
      var outputString = util.inspect(
        object, util._extend({ customInspect: false }, options));
      // Write to logger
      logger.log(logger.logLevels.verbose, outputString + '\n');
      // Write to stdout incase it was overridden
      process.stdout.write(outputString + '\n');
      // Write to Visual Studio output window if debugger is attached
      if (Debug.debuggerEnabled) {
    ...
    console['timeEnd'] = function(label) {
      ...
      var outputString = util.format('%s: %dms', label, duration);
      // Write to logger
      logger.log(logger.logLevels.verbose, outputString);
      // Write to stdout incase it was overridden
      process.stdout.write(outputString + '\n');
  ...
  function setupGlobalConsole() {
    ...
    // Dummy console for UWP applications
    var emptyConsole = {
      log: function() { },
      debug: function() { },
      info: function() { },
